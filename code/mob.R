...
# x   : name of X in the input dataframe with numeric values
  ...
  repeat {
    pts <- unique(quantile(df1[[xname]], prob = seq(0, 1, 1 / (nbin + 1)), names = F))
    ...
    if((flg1 + flg2) > 0) {
      cuts <- unique(quantile(df1[[xname]], prob = seq(0, 1, 1 / nbin), names = F))
      break
    }
    nbin <- nbin + 1
  }
...
# x   : name of X in the input dataframe with numeric values
  ...
  repeat {
    pts <- unique(quantile(df2[[xname]], prob = seq(0, 1, 1 / (nbin + 1)), names = F))
    df1$cut <- cut(df1[[xname]], breaks = pts, include.lowest = T)
    df3 <- Reduce(rbind, 
             Map(function(x) data.frame(xmean = mean(x[[xname]]), 
			                ymean = mean(x[[yname]])), 
    ...
    if((flg1 + flg2) > 0) {
      cuts <- unique(quantile(df2[[xname]], prob = seq(0, 1, 1 / nbin), names = F))
      break
    }
    nbin <- nbin + 1
  }
